...
export async function handleStripeWebhook(rawBody: Buffer, signatureHeader: string | undefined) {
  ...
  if (event.type === "checkout.session.completed" && object.payment_status === "paid") await markOrderPaid(orderId, String(object.payment_intent ?? object.id), event.id);
  if (event.type === "payment_intent.payment_failed") await markOrderPaymentFailed(orderId, String(object.id), event.id);
