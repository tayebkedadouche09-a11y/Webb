...
export const appRouter = router({
  ...
  orders: router({
    ...
    checkout: protectedProcedure.input(z.object({ orderId: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
      ...
      const session = await createStripeCheckoutSession({ orderId: row.order.id, name: row.item.productName, amount: String(row.item.unitPrice), currency: row.order.currency, customerEmail: row.user.email });
      await recordCheckoutSession(ctx.user.id, row.order.id, session.sessionId);
      return session;
    }),
  }),
  purchases: router({
